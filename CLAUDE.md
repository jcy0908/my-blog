...
마크다운 기반 블로그+미니 웹앱 포트폴리오. 기본은 HTML, CSS, JavaScript다.

계산 규칙을 C++로 쓰고 WebAssembly로 빌드해 쓸 수 있다(예: apps/2048의
이동·병합 규칙). 조건이 있다.

- Emscripten 같은 무거운 툴체인을 끌어오지 않는다. clang의 wasm32 타깃과
  wasm-ld만으로 빌드하고, 산출물은 .wasm 하나여야 한다.
- 받지 못했을 때를 대비해 같은 API의 JS 구현을 반드시 남긴다.
- 두 구현이 같은 값을 낸다는 것을 검증하는 테스트를 함께 둔다.
